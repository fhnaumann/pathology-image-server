...
parser.add_argument("--username", default="orthanc", help="Username to the REST API")

parser.add_argument("--password", default="orthanc", help="Password to the REST API")

parser.add_argument(
    "--force", help="Do not warn the user about deletion", action="store_true"
)

parser.add_argument(
    "--clear", help="Remove the content of the Orthanc database", action="store_true"
)
...
parser.add_argument("files", metavar="N", nargs="*", help="Files to import")
...
def UploadBuffer(dicom):
    ...
    if isinstance(info, dict) and not info["ParentStudy"] in IMPORTED_STUDIES:
        IMPORTED_STUDIES.add(info["ParentStudy"])

        r2 = requests.get(
            "%s/instances/%s/tags?short" % (args.url, info["ID"]), auth=auth
        )
...
def UploadFile(path):
    print("uploading with path ", path)
    ...
    with tarfile.open(path, decoder) as tar:
        for item in tar:
            if item.isreg():
                ...
                if args.verbose:
                    print(
                        "Uploading: %s (%dMB)" % (item.name, len(dicom) / (1024 * 1024))
                    )
        ...
        for item in zip.infolist():
            ...
            if item.file_size > 0:
                ...
                if args.verbose:
                    print(
                        "Uploading: %s (%dMB)"
                        % (item.filename, len(dicom) / (1024 * 1024))
                    )
...
if args.clear:
    ...
    for study in r.json():
        requests.delete(
            "%s/studies/%s" % (args.url, study), auth=auth
        ).raise_for_status()
